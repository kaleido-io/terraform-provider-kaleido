...
//create paladin stacks - originator 
resource "kaleido_platform_network" "paladin_network_og" {
  provider = kaleido.originator
  type        = "PaladinNetwork"
  name        = "paladinnetwork_og"
  environment = kaleido_platform_environment.env_og.id
  config_json = jsonencode({
    type = "evmRegistry"
    evmRegistry = {
...
resource "kaleido_platform_stack" "paladin_stack_og" {
  provider = kaleido.originator
  environment = kaleido_platform_environment.env_og.id
  name        = "paladin_stack_og"
  type        = "chain_infrastructure"
  sub_type    = "PaladinStack"
  network_id  = kaleido_platform_network.paladin_network_og.id
}

resource "kaleido_platform_runtime" "paladin_runtime_og" {
  provider = kaleido.originator
  type         = "PaladinNodeRuntime"
  name         = "paladin_runtime_og"
  environment  = kaleido_platform_environment.env_og.id
  config_json  = jsonencode({})
  stack_id     = kaleido_platform_stack.paladin_stack_og.id
  size = "Large"
  zone = var.originator_peer_network_dz
}

resource "kaleido_platform_service" "paladin-service-og" {
  provider = kaleido.originator
  type        = "PaladinNodeService"
  name        = "paladin-service-og"
  environment = kaleido_platform_environment.env_og.id
  runtime     = kaleido_platform_runtime.paladin_runtime_og.id
  config_json = jsonencode({
    network = {
      id = kaleido_platform_network.paladin_network_og.id
    },
    baseLedgerEndpoint = {
      type = "local"
      local = {
        node = {
          id = kaleido_platform_service.bns_signer_net_og[0].id
        }
      }
    },
...
//create paladin stacks, first joiner
resource "kaleido_platform_network" "paladin_network_j1" {
  provider = kaleido.joiner_one
  type        = "PaladinNetwork"
  name        = "paladinnetwork_j1"
  environment = kaleido_platform_environment.env_j1.id
  config_json = jsonencode({
    type = "evmRegistry"
    evmRegistry = {
...
resource "kaleido_platform_stack" "paladin_stack_j1" {
  provider = kaleido.joiner_one
  environment = kaleido_platform_environment.env_j1.id
  name        = "paladin_stack_j1"
  type        = "chain_infrastructure"
  sub_type    = "PaladinStack"
  network_id  = kaleido_platform_network.paladin_network_j1.id
}

resource "kaleido_platform_runtime" "paladin_runtime_j1" {
  provider = kaleido.joiner_one
  type         = "PaladinNodeRuntime"
  name         = "paladin_runtime_j1"
  environment  = kaleido_platform_environment.env_j1.id
  config_json  = jsonencode({})
  stack_id     = kaleido_platform_stack.paladin_stack_j1.id
  size = "Large"
  zone = var.joiner_one_peer_network_dz
}

resource "kaleido_platform_service" "paladin-service-j1" {
  provider = kaleido.joiner_one
  type        = "PaladinNodeService"
  name        = "paladin-service-j1"
  environment = kaleido_platform_environment.env_j1.id
  runtime     = kaleido_platform_runtime.paladin_runtime_j1.id
  config_json = jsonencode({
    network = {
      id = kaleido_platform_network.paladin_network_j1.id
    },
    baseLedgerEndpoint = {
      type = "local"
      local = {
        node = {
          id = kaleido_platform_service.bns_peer_net_j1[0].id
        }
      }
    },
    baseConfig            = "{\"blockIndexer\":{\"fromBlock\": 0 }}",
...
//create paladin stacks - second joiner
resource "kaleido_platform_network" "paladin_network_j2" {
  provider = kaleido.joiner_two
  type        = "PaladinNetwork"
  name        = "paladinnetwork_j2"
  environment = kaleido_platform_environment.env_j2.id
  config_json = jsonencode({
    type = "evmRegistry"
    evmRegistry = {
...
resource "kaleido_platform_stack" "paladin_stack_j2" {
  provider = kaleido.joiner_two
  environment = kaleido_platform_environment.env_j2.id
  name        = "paladin_stack_j2"
  type        = "chain_infrastructure"
  sub_type    = "PaladinStack"
  network_id  = kaleido_platform_network.paladin_network_j2.id
}

resource "kaleido_platform_runtime" "paladin_runtime_j2" {
  provider = kaleido.joiner_two
  type         = "PaladinNodeRuntime"
  name         = "paladin_runtime_j2"
  environment  = kaleido_platform_environment.env_j2.id
  config_json  = jsonencode({})
  stack_id     = kaleido_platform_stack.paladin_stack_j2.id
  size = "Large"
  zone = var.joiner_two_peer_network_dz
}

resource "kaleido_platform_service" "paladin-service-j2" {
  provider = kaleido.joiner_two
  type        = "PaladinNodeService"
  name        = "paladin-service-j2"
  environment = kaleido_platform_environment.env_j2.id
  runtime     = kaleido_platform_runtime.paladin_runtime_j2.id
  config_json = jsonencode({
    network = {
      id = kaleido_platform_network.paladin_network_j2.id
    },
    baseLedgerEndpoint = {
      type = "local"
      local = {
        node = {
          id = kaleido_platform_service.bns_peer_net_j2[0].id
        }
      }
    },
    baseConfig            = "{\"blockIndexer\":{\"fromBlock\": 0 }}",
...
//Now create the networks between the the different accounts j1/og
resource "kaleido_network_connector" "paladin_j1_connector_og" {
  provider = kaleido.joiner_one
  type = "Platform"
  name = "paladin_${var.joiner_one_name}_conn_${var.originator_name}"
  environment = kaleido_platform_environment.env_j1.id
  network = kaleido_platform_network.paladin_network_j1.id
  zone = var.joiner_one_peer_network_dz

  platform_requestor = {
    target_account_id = data.kaleido_platform_account.acct_og.account_id
    target_environment_id = kaleido_platform_environment.env_og.id
    target_network_id = kaleido_platform_network.paladin_network_og.id
  }
}


resource "kaleido_network_connector" "paladin_og_connector_j1" {
  provider = kaleido.originator
  type = "Platform"
  name = "paladin_${var.originator_name}_conn_${var.joiner_one_name}"
  environment = kaleido_platform_environment.env_og.id
  network = kaleido_platform_network.paladin_network_og.id
  zone = var.originator_peer_network_dz
  
  platform_acceptor = {
    target_account_id = data.kaleido_platform_account.acct_j1.account_id
    target_environment_id = kaleido_platform_environment.env_j1.id
    target_network_id = kaleido_platform_network.paladin_network_j1.id
    target_connector_id = kaleido_network_connector.paladin_j1_connector_og.id
  }
}


//Now create the networks between the the different accounts - j2/og
resource "kaleido_network_connector" "paladin_j2_connector_og" {
  provider = kaleido.joiner_two
  type = "Platform"
  name = "paladin_${var.joiner_two_name}_conn_${var.originator_name}"
  environment = kaleido_platform_environment.env_j2.id
  network = kaleido_platform_network.paladin_network_j2.id
  zone = var.joiner_one_peer_network_dz
...
resource "kaleido_network_connector" "paladin_og_connector_j2" {
  provider = kaleido.originator
  type = "Platform"
  name = "paladin_${var.originator_name}_conn_${var.joiner_two_name}"
  environment = kaleido_platform_environment.env_og.id
  network = kaleido_platform_network.paladin_network_og.id
  zone = var.originator_peer_network_dz
  ...
  platform_acceptor = {
    target_account_id = data.kaleido_platform_account.acct_j2.account_id
    target_environment_id = kaleido_platform_environment.env_j2.id
    target_network_id = kaleido_platform_network.paladin_network_j2.id
    target_connector_id = kaleido_network_connector.paladin_j2_connector_og.id
  }
}