...
resource "kaleido_platform_network_connection" "conn_0" {
  network = net_0.id
  name = "conn0"
  environment = kaleido_platform_environment.env_0.id
  config_json = jsonencode({
...
resource "kaleido_platform_network" "net_1" {
  type = "Besu"
  name = "evmchain1"
  environment = kaleido_platform_environment.env_0.id
  config_json = jsonencode({
    chainID = net_0.config.chainID
    externalGenesis = true
  })
}

resource "kaleido_platform_network_connection" "conn_1" {
  network = net_1.id
  name = "conn1"
  environment = kaleido_platform_environment.env_0.id
  config_json = jsonencode({
